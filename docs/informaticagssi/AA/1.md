...
Unfortunately, many optimization problems are $\mathsf{NP}$-hard.

Unless $\mathsf{P}=\mathsf{NP}$, we cannot devise **efficient** (polynomial time) algorithms to find **optimal solutions** to such problems that at the same time:
