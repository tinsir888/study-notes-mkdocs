...
If there exists a $c\ln n$-approximation algorithm for the unweighted set cover problem for some constant $c<1$, then there is an $\mathsf{DTIME}(n^{O(\log\log n)})$-time algorithm for each NP-complete problem. (2014: then $\mathsf{P}=\mathsf{NP}$)
