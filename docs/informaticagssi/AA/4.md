...
Theorem: There is no $\alpha$-approximation algorithm for the $k$-center problem for a $\alpha<2$, unless $\mathsf{P}=\mathsf{NP}$.
...
The problem is $\mathsf{NP}$-hard. Even deciding if there exists a schedule for which $L_\max\leq0$ is strongly $\mathsf{NP}$-hard.
...
- As deciding if there exists a schedule for which $L_\max\leq0$ is strongly $\mathsf{NP}$-hard, this implies that $\mathsf{P}=\mathsf{NP}$.
