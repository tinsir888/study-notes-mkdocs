...
A labeled graph is a triple $G=(V,E,\ell)$ where $\ell:V\cup E\rightarrow L$ is a labeling function assigning a label from a set of labels $L$ to each node and each edge.
...
A graph $G_1(V_1,E_1,\ell_1)$ is **isomorphic** 同构 to a graph $G_2=(V_2,E_2,\ell_2)$ if there exists a bijective function $f:V_1\rightarrow V_2$ such that if $u\in V_1$ then $f(u)\in V_2$ and $\ell_1(u)=\ell_2(f(u))$ and if $(u,v)\in E_1$ then $(f(u),f(v))\in E_2$ and $\ell_1((u,v))=\ell_2((f(u),f(v)))$.
...
!!! success
    ...
    :thumbsup: Finds all frequent subgraphs.

    :thumbsup: Easy to include heuristics to reduce the computation time.



!!! failure

    :thumbsdown: FSG generates the same subgraph a large number of times.
    :thumbsdown: The canonical labeling scheme is quadratic in the size of the subgraph.
    :thumbsdown: FSG requires several isomorphism and subgraph isomorphism tests.
...
1. $i_1=i_2$ and $j_1\lt j_2\Rightarrow e_1\prec e_2$ backward edges ordering
2. $i_1\lt j_1$ and $j_1=i_2\Rightarrow e_1\prec e_2$ forward edges ordering
3. $e_1\prec e_2$ and $e_2\prec e_3\Rightarrow e_1\prec e_3$ transitive property
...
- $\exists t,0\le t\le\min(n,m)$ s.t. $a_k=b_k$ for $k\lt t$ and $a_t\prec_e b_t$
...
!!! success
    ...
    :thumbsup: Generates less redundant subgraphs.



!!! failure

    :thumbsdown: Works only in collections of graphs.

    :thumbsdown: Generally slow for low $\min\sup$.
...
1. Construct an overlap graph $G_O=(V_O,E_O)$, in which the set of nodes $V_O$ is the set of matches of a pattern $P$ into a graph $G$ and $E_O=\{(f_1,f_2)|f_1,f_2\in V_O\lor f_1\neq f_2\neq f_1\sqcap f_2\neq\emptyset\}$, i.e., $E_O$ has an edge among each pair of overlapping matches.
